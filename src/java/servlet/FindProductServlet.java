...
import tblProduct.ProductDAO;
import tblProduct.ProductDTO;
...
public class FindProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int proId = Integer.parseInt(request.getParameter("txtProductId")) ;
            ProductDAO pDAO = new ProductDAO();
            CategoryDAO cDAO = new CategoryDAO();
            List<CategoryDTO> cList = cDAO.getCategoryList();
            request.setAttribute("CATEGORY_LIST", cList);

            ProductDTO dto = pDAO.getProductForAdmin(proId);
            
            request.setAttribute("PRODUCT", dto);
           
        } catch (SQLException ex) {
            log("FindProductServlet_SQL " + ex.getMessage());
        } catch (NamingException ex) {
            log("FindProductServlet_Naming " + ex.getMessage());
        } finally {
